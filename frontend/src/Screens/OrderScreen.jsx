  ...
  const { success: successTracking } = orderTracking;
    ...
    useEffect(() => {
      ...
      const addPayPalScript = async () => {
        ...
        script.src = `https://www.paypal.com/sdk/js?client-id=${data}&currency=${currency}`;
        script.async = true;
        script.onload = () => {
          setSdkReady(true);
        };
            ...
            <Card>
              <ListGroup variant='flush'>
                ...
                {!order.isPaid && (
                  <ListGroup.Item>
                    {loadingPay && <Loader />}
                    {!sdkReady ? <Loader /> : <PayPalButton amount={order.total} onSuccess={successPaymentHandler} options={{ currency }} currency={currency} />}
                  </ListGroup.Item>
                )}
